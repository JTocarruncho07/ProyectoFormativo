...
// Consulta principal de máquinas
...
<div class="container-fluid px-3 py-4">
    <h2 class="text-center mb-4">Control de Máquinas</h2>
                ...
                <tr>
                    ...
                    <td>
                        <div class="d-flex justify-content-center">
                            ...
                            <button class="btn btn-warning" id="btn-reiniciar-<?php echo $maquina['id']; ?>" onclick="reiniciarTiempo(<?php echo $maquina['id']; ?>)" disabled>Reiniciar</button>
                        </div>
                    </td>
                    <td>
                        <div class="d-flex justify-content-center">
                            <button class="btn btn-info" onclick="mostrarHistorial(<?php echo $maquina['id']; ?>)">Ver Historial</button>
                        </div>
                    </td>
                </tr>
                <?php endforeach; ?>
...
window.addEventListener('load', function() {
    <?php foreach ($maquinas as $maquina): ?>
        let id = <?php echo $maquina['id']; ?>;
        if (tiempos[id] && tiempos[id].activo) {
            // Si el cronómetro estaba activo, reanudar
            iniciarPausar(id);
        } else if (tiempos[id]) {
            // Si hay tiempo guardado pero no está activo, mostrar el tiempo
            document.getElementById(`horas-${id}`).textContent = formatTime(tiempos[id].tiempo);
        }
    <?php endforeach; ?>
});
...
function iniciarPausar(id) {
    ...
    let btnReiniciar = document.getElementById(`btn-reiniciar-${id}`);
    
    if (!tiempos[id]) {
        tiempos[id] = { activo: false, tiempo: 0 };
    }
    
    if (!cronometros[id]) {
        // Si no está en ejecución, iniciar
